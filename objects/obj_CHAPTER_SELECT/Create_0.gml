...
create_start_screen = function()
{
	...
	var choices = [new create_choice(yes_text, (0 << 0)), new create_choice(no_text, (1 << 0))];
...
create_start_next_screen = function()
{
	...
	var choices = [new create_choice(play_next_text, (0 << 0)), new create_choice(chapter_select_text, (1 << 0))];
	var start_screen = instance_create(0, 0, obj_screen_start);
...
trigger_event = function(arg0, arg1)
{
	...
	switch (_current_state)
	{
		...
		case (4 << 0):
			...
			else if (event_value == (4 << 0))
			{
				quit();
			}
			else if (event_value == (5 << 0))
			{
				toggle_language();
			}
			break;
